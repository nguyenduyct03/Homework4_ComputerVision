...
# 	### YOUR CODE HERE
# 	for y in range(1,H):
# 		paths[y,0] = np.argmin(cost[y-1,0:2])
# 		cost[y,0] = (cost[y-1,0:2])[paths[y,0]]+energy[y,0]
# 		paths[y,W-1] = np.argmin(cost[y-1,-2:])-1
# 		cost[y,W-1] = (cost[y-1,-2:])[paths[y,W-1]+1]+energy[y,W-1]
# 		for x in range(1,W-1):
# 			paths[y,x] = np.argmin(cost[y-1,x-1:x+2])-1
# 			cost[y,x] = (cost[y-1,x-1:x+2])[paths[y,x]+1]+energy[y,x]
# 	### END YOUR CODE
...
def compute_forward_cost(image, energy):
	...
	### YOUR CODE HERE
	for i in range(1,H):
		left_nb = np.insert(image[i,0:W-1],0,image[i,1])
		right_nb = np.insert(image[i,1:W],W-1,image[i,-2])
		C_middle = np.abs(right_nb - left_nb)
		C_left = C_middle + np.abs(image[i-1]-left_nb)
		C_right = C_middle + np.abs(image[i-1]-right_nb)
		e_left_nb = np.insert(cost[i-1,0:W-1],0,1e10)
		e_middle = cost[i-1]
		e_right_nb = np.insert(cost[i-1,1:W],W-1,1e10)
		L = e_left_nb + C_left
		M = e_middle + C_middle
		R = e_right_nb + C_right
		pixel_neighbors = np.vstack([L,M,R])
		cost[i] = energy[i] + np.min(pixel_neighbors, axis=0)
		paths[i] = np.argmin(pixel_neighbors,axis=0)-1
